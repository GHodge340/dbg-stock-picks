...
async function fetchLogContent() {
    ...
    if (source === 'REMOTE') {
        console.log(`📡 Connecting to AWS EC2 (${process.env.EC2_HOST})...`);
        try {
            const keyPath = process.env.EC2_KEY_PATH || './kali_key.pem';
            ...
            const remotePath = process.env.EC2_LOG_PATH || '/home/ubuntu/alpacaTrader/engine_log.txt';
            console.log(`📥 Downloading log from ${remotePath}...`);
        ...
        if (!fs.existsSync(localPath)) {
            console.error("❌ Error: engine_log.txt not found locally.");
            return null;
        }
...
async function getCompanyContext(symbol) {
    try {
        // Fetch news and summary profile for context
        ...
        const description = profile.summaryProfile?.longBusinessSummary?.slice(0, 300) + '...';
...
async function generateWatchlist() {
    ...
    // Process top 5 candidates with AI
    const finalReport = [];
...
async function sendDiscordReport(report) {
    ...
    if (!webhookUrl) {
        console.warn("⚠️ Discord Webhook URL not found. Skipping alert.");
        return;
    }
    ...
    const embeds = report.map(c => ({
        ...
        fields: [
            ...
            { name: "💡 AI Thesis", value: c['AI Thesis'], inline: false },
    ...
    try {
        // Discord allows up to 10 embeds per message. We are sending 5.
...
function saveToJson(data) {
    ...
    try {
        ...
        try {
            // These commands assume you have initialized git in the project root
            ...
            console.warn("⚠️ Git Push skipped or failed. (Make sure you are in a git repo and authenticated)");
        }
...
// === SCHEDULER ===
// 10:00 AM EST (Hour 10) and 3:00 PM EST (Hour 15)
